...
const ShimmerCard = React.forwardRef<HTMLDivElement, { children: React.ReactNode, className?: string }>(({ children, className, ...props }, ref) => {
    return (
        <div
            ref={ref}
            className={`relative overflow-hidden rounded-xl border border-white/10 bg-zinc-900/80 shadow-lg ${className}`}
            {...props}
        >
...
export function AuthDialog({ open, onOpenChange }: AuthModalProps) {

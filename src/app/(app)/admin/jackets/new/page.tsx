...
const jacketFormSchema = z.object({
  vin: z.string().min(6, "VIN must be at least 6 characters").max(17, "VIN cannot exceed 17 characters"),
  ...
  buyerFee: z.coerce.number().min(0).optional(),
  onlineFee: z.coerce.number().min(0).optional(),
  managementFee: z.coerce.number().min(0).default(100),
  auctionInvoiceTotal: z.coerce.number().min(0).optional(),
...
export default function NewJacketPage() {
  ...
  const onSubmit = async (data: JacketFormValues) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        form.setError("vin", { message: "A jacket with this VIN already exists." });
        setLoading(false);
        return;
      }
      ...
      const payload = {
        ...
        jacketId: `J${Date.now()}`,
        vin: normalizedVin,
        auctionSaleDate: Timestamp.fromDate(new Date()),
        isAuctionPaid: false,
        isMgmtFeePaid: false,
        miscFees: [],
        documents: [],
        ...
        <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-muted-foreground">
                Create a jacket manually or by uploading an auction invoice to be parsed.
              </p>
            </div>
            <TabsList>
              <TabsTrigger value="manual"><FilePlus2 className="mr-2"/>Manual Entry</TabsTrigger>
              <TabsTrigger value="upload"><FileUp className="mr-2"/>Upload Invoice</TabsTrigger>
            </TabsList>
        </div>
        ...
        <TabsContent value="manual">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              <div className="flex justify-end items-center gap-2 sticky top-0 py-2 z-10 -mt-4">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                             <Label className="flex items-center gap-1">
                              Auction Invoice Total
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Info className="h-3 w-3 text-muted-foreground" />
                                </TooltipTrigger>
                                <TooltipContent>
                                  <p>The full amount listed on the original auction invoice.</p>
                                </TooltipContent>
                              </Tooltip>
                            </Label>
                            <FormControl>

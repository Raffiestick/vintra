...
type PacketInput = { vin?: string };

export const generateJacketPacket = onCall({ region: "us-central1" }, async (req) => {
  const { vin: rawVin } = (req.data || {}) as PacketInput;
  if (!rawVin) {
  ...
  for (const d of Array.isArray(j.documents) ? j.documents : []) {
    try {
      ...
      if (ct.startsWith("application/pdf")) {
        ...
        const page = packet.addPage([595.28, 841.89]);

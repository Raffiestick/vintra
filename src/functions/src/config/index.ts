...
export async function extractPdfText(buf: Buffer): Promise<string> {
  if (buf.length > MAX_PDF_SIZE_BYTES) {
    throw new HttpsError("invalid-argument", `File is too large. Maximum size is ${MAX_PDF_SIZE_BYTES / 1024 / 1024}MB.`);
  }
  ...
  try {
    ...
    throw new HttpsError("internal", "Could not parse the PDF file. It may be corrupted or in an unsupported format.", error.message);
  }
}

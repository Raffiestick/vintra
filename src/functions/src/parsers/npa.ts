...
function detectUnitType(block: string): "Motorcycle" | "Boat" | "Other" {
...
export function parseNpaInvoiceText(text: string): {
  ...
  const out = {
    ...
    invoiceMeta: {} as { aucNo?: string; saleLocation?: string },
    units: [] as any[],
  };
  ...
  for (let i = 0; i < vins.length; i++) {
    ...
    const ymm = chunk.match(
      /(?:^|\s)(\d{4})\s+([A-Z][A-Z-]+)\s+([A-Z0-9][A-Z0-9 /()'*\-\.]+?)(?=\s+(?:SALE LOC:|[A-Z]{2}\s+TITLE|Item Price|PRICE|Sub-Total))/,
    );
    if (ymm) {
      unit.year = +ymm[1];
      unit.make = ymm[2];
    ...
    const stock =
      pre.match(/STOCK#\s*(\d{5,})/i) || chunk.match(/STOCK#\s*(\d{5,})/i);

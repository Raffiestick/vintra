...
function parseNpaInvoiceText(text) {
    const out = { invoiceDate: pickInvoiceDateFromHeader(text), invoiceMeta: {}, units: [] };
    ...
    if (!out.invoiceMeta?.saleLocation) {
        ...
        if (headerSale)
            out.invoiceMeta = { ...(out.invoiceMeta || {}), saleLocation: headerSale[1].trim() };
    }
    return out;
}
/* ───────────────────── Auth Callables ───────────────────── */
...
"""${text.slice(0, 20000)}"""`;
                ...
                if (!extracted.invoiceDate)
                    extracted.invoiceDate = guessInvoiceDateFromText(text);
            }
        }
        // If still no units, at least seed with VINs so the batch is never empty
        if (!extracted.units.length) {
            for (const vin of extractVinsFromText(text))

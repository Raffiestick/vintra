...
export function parseNpaInvoiceText(text: string): {
   ...
   */
  const VIN_OR_HIN_RE =
    /(?<![A-Z0-9])(?:[A-HJ-NPR-Z0-9]{17}|[A-Z0-9]{12})(?![A-Z0-9])/g;
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const end =
      i < matches.length - 1 ? (matches[i + 1].index ?? norm.length) : norm.length;
    ...
    const unit: any = { vinOrHin: id };
    if (id.length === 17) unit.vin = id;
    if (id.length === 12) unit.hin = id; // boats

    // ---- Year Make Model ---- (very stable in invoice block)
    // ex: "2012 NAUTIC STAR BOATS 203SC SPORT DECK"
    const ymm =
      chunk.match(
        /(?:^|\n|\s)(\d{4})\s+([A-Z][A-Z- ]+?)\s+([A-Z0-9][A-Z0-9 /()'*\-+]+?)(?=\n| SALE LOC:| [A-Z]{2}\s+TITLE|\bREPO\b|\bBANK OWNED\b|\bBOS ONLY\b| Item Price| PRICE| Sub-Total)/,
    ...
    if (len) unit.lengthFeet = +len[1];

    const eng = /E\s*=\s*([A-Z0-9\-+\/\s]+?)(?=\s|$|\[)/i.exec(nearBlock);
    if (eng) unit.engine = eng[1].replace(/\s+/g, " ").trim();
    ...
    const title =
      /(REPO(?:\s+TITLE)?|BANK OWNED|FL\s+TITLE|GA\s+TITLE|OH\s+TITLE(?:\s*\/\s*TRL)?|ME\s+TITLE|SC\s+TITLE|BOS\s+ONLY|LEGAL\s+DOCS)/i.exec(
        titleBlock,
      );
    if (title) unit.titleInfo = title[1].replace(/\s+/g, " ").toUpperCase();

    // ---- SALE LOC ----
    const saleM =
      chunk.match(/SALE LOC:\s*([A-Z ]{3,})/i) ||
      pre.match(/SALE LOC:\s*([A-Z ]{3,})/i);
    if (saleM) unit.saleLocation = saleM[1].replace(/\s+/g, " ").trim();

    // ---- STOCK# and AUC# ----
    const stockM =
      pre.match(/STOCK#\s*([0-9]{5,})/i) || chunk.match(/STOCK#\s*([0-9]{5,})/i);
    if (stockM) unit.stockNo = stockM[1];

    const aucM =
      pre.match(/\bAUC#\s*([0-9]+)\b/i) || chunk.match(/\bAUC#\s*([0-9]+)\b/i);
    ...
    const priceM =
      chunk.match(/Item Price:\s*\$?\s*([\d,]+(?:\.\d{2})?)/i) ||
      chunk.match(/Item Price:\s*\n\s*\$?\s*([\d,]+(?:\.\d{2})?)/i);
    const bfeeM =
      chunk.match(/Buyer Fee:\s*\$?\s*([\d,]+(?:\.\d{2})?)/i) ||
      chunk.match(/Buyer Fee:\s*\n\s*\$?\s*([\d,]+(?:\.\d{2})?)/i);
    const ofeeM =
      chunk.match(/Online Fee:\s*\$?\s*([\d,]+(?:\.\d{2})?)/i) ||
    ...
    if (!unit.itemPrice && !unit.buyerFee && !unit.onlineFee) {
      ...
      if (subM) unit.itemPrice = numFrom(subM[1]); // editable later in Jacket UI
    }

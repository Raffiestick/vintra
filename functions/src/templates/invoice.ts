import type { JacketData, Company, Party } from "../types"; // Now imports from the master types file

const fmtUSD = (n: any) => Number(n || 0).toLocaleString("en-US", { style: "currency", currency: "USD" });

export function renderInvoiceHTML(j: JacketData, seller: Company, buyer: Party) {
    const itemPrice = Number(j.itemPrice || 0);
    const buyerFee = Number(j.buyerFee || 0);
    const onlineFee = Number(j.onlineFee || 0);
    const mgmtFee = Number(j.managementFee || 0);
    const miscFees = Array.isArray(j.miscFees) ? j.miscFees : [];

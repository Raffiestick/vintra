...
const fmtDate = (d: Date | Timestamp | string | null | undefined): string => {
    if (!d) return "";
    if (d instanceof Timestamp) {
        return d.toDate().toLocaleDateString("en-US", { timeZone: 'UTC' });
    }
...
export function renderReassignmentHTML(jacketData: JacketData, seller: Company, buyer: Party): string {
    const {
        vin, year, make, model, color, odometer, itemPrice, buyerFee,
    ...
    <tr><th>Body Type / Color</th><td>${[jacketData.bodyType, color].filter(Boolean).join(" / ")}</td></tr>
    ...
    <tr><th>State of Issue</th><td>${jacketData.titleState || ""}</td></tr>
    ...
    <tr><th>Auction Through Which Vehicle Was Sold</th><td>${jacketData.saleLocation || ""}</td></tr>
    </table>

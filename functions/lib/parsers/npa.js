...
function detectUnitType(block) {
    const b = block.toUpperCase();
    if (/HURRICANE|MERC|YAM|NAUTICSTAR|L=\d+|E=\d+|HOURS?\b/.test(b))
        return "Boat";
    if (/HARLEY|HONDA|KAWASAKI|SUZUKI|YAMAHA|ROAD KING|STREET GLIDE|SPORTSTER|FLH|VIN/.test(b))
        return "Motorcycle";
    return "Other";
}
function parseNpaInvoiceText(text) {
    ...
    for (let i = 0; i < vins.length; i++) {
        ...
        const unit = { vin: v[0].toUpperCase() };
        // Year / Make / Model
        ...
        if (color)
            unit.color = color[1].replace(/\s+/g, "").toUpperCase();
        ...
        if (eng)
            unit.engine = eng[1].trim();
        // Title info
        const t = chunk.match(/([A-Z]{2})\s+TITLE(?:\s*\/\s*TRL)?/i);
        if (t)
            unit.titleInfo = t[0].replace(/\s+/g, " ").toUpperCase();
        // Trailer presence
        if (/\/\s*TRL\b|TRAILER\b/i.test(chunk))
            unit.hasTrailer = true;
        // Sale location
        const sale = chunk.match(/SALE LOC:\s*([A-Z ]{3,})/i);
        if (sale)
            unit.saleLocation = sale[1].replace(/\s+/g, " ").trim();
        // Stock# (may be before VIN)
        const stock = pre.match(/STOCK#\s*(\d{5,})/i) || chunk.match(/STOCK#\s*(\d{5,})/i);
        if (stock)
            unit.stockNo = stock[1];
        // Fees
        ...
        if (price)
            unit.itemPrice = parseFloat(price[1].replace(/,/g, ""));
        if (bfee)
            unit.buyerFee = parseFloat(bfee[1].replace(/,/g, ""));
        if (ofee)
            unit.onlineFee = parseFloat(ofee[1].replace(/,/g, ""));
    ...
    if (!out.invoiceMeta?.saleLocation) {
        ...
        if (headerSale)
            out.invoiceMeta.saleLocation = headerSale[1].trim();
    }
    // Normalize header date
    if (out.invoiceDate)
        out.invoiceDate = (0, config_1.normalizeIsoFromDateLike)(out.invoiceDate);
    return out;
}

...
function parseNpaInvoiceText(text) {
    var _a, _b, _c, _d;
    const out = { invoiceDate: (0, config_2.pickInvoiceDateFromHeader)(text), invoiceMeta: {}, units: [] };
    ...
    for (let i = 0; i < matches.length; i++) {
        const v = matches[i];
        const start = (_a = v.index) !== null && _a !== void 0 ? _a : 0;
        const end = i < matches.length - 1 ? ((_b = matches[i + 1].index) !== null && _b !== void 0 ? _b : norm.length) : norm.length;
    ...
    if (!((_c = out.invoiceMeta) === null || _c === void 0 ? void 0 : _c.saleLocation)) {
        const preHeaderSale = norm.slice(0, matches.length ? ((_d = matches[0].index) !== null && _d !== void 0 ? _d : 0) : 2000);
        const headerSale = preHeaderSale.match(/SALE LOC:\s*([A-Z ]{3,})/i);
        if (headerSale)
            out.invoiceMeta = Object.assign(Object.assign({}, (out.invoiceMeta || {})), { saleLocation: headerSale[1].trim() });
    }
    return out;
}
